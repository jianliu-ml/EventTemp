            ...
            for _ in range(len(data_y)):  ###
                ...
                for p in arg_sort:  ## position
                    if not p in alread_predicted:
                        ...
                        example_result.append([p, p_label.item()])
...
if __name__ == '__main__':
    ...
    model.to(device)
    load_model(model, 'models/best_model')
    ...
    model.eval()
    n_gold, n_predict, n_correct, precision, recall, f1 = eval(model, test_dataset, device, n_test_relation)